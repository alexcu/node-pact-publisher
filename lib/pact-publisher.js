...
PactPublisher.prototype.getPacticipantNames = function () {
  return Q.Promise(function (resolve, reject) {
    request({
      uri: this._pactBrokerBaseUrl + "/pacticipants",
      method: 'GET',
      json: true
...
/**
 * Checks if a pact has been registered
 * @param  {String} name Pact name to check
 * @return {Promise}     Promise to if the pact is registered
 */
PactPublisher.prototype.isPactRegistered = function (name) {
  return Q.Promise(function (resolve, reject) {
    this.getPacticipantNames().then(function (names) {
...
PactPublisher.prototype.registerPact = function (name) {
  return Q.Promise(function (resolve, reject) {
    request({
      ...
      if (err || res.statusCode !== 200 || body.name === undefined) {
        reject(err || body);
      } else {
        resolve(body);
      }
...
PactPublisher.prototype._publish = function (pact) {
  ...
  request({
    ...
    if (err || res.statusCode !== 200) {
      var msg = 'Error uploading pact file ' + pactFile + ' to broker';
 ...
 *                                  parameter.
 * @return {Number}                 Number of pacts published
 */
PactPublisher.prototype.publish = function (pactFileName) {
  var _runPublish = function (pacts) {
    var publishCount = 0;
    return Q.Promise(function (resolve, reject) {
      pacts.forEach(function (pact) {
        this._log('Publishing \'' + pact.pactFileName + '\'...');
        this._publish(pact).then(function () {
          this._log('\'' + pact.pactFileName + '\' published to broker!', 'mark');
          if (++publishCount === pacts.length) {
            resolve(publishCount);
          }
        }, function (err) {
          this._log('Failed to publish \'' + pact.pactFileName + '\': ');
          reject(publishCount);
        });
      });
    });
  ...
  this._enqueuedPactFiles.forEach(function (pactFileName) {
    this._loadPactFile(pactFileName).then(function (pactObject) {
      pactObject.pactFileName = pactFileName;
      pactsToPublush.push(pactObject);
      this._log('Loaded pact contract \'' + pactFileName + '\'');
      if (pactsToPublush.length === this._enqueuedPactFiles.length) {
        _runPublish(pacts).then(function (publishCount) {
          this._log('Successfully published all ' + publishCount + ' pact contracts!', 'mark');
        }, function (publishCount) {
          this._log('Failed to publish all pact contracts. Only published ' + publishCount + ' contracts.', 'error');
        });
      }
    }, function (err) {
      this._log('Failed to load pact contract \'' + pactFileName + '\': ' + err, 'error');
      return 0;
    });
  });
};
